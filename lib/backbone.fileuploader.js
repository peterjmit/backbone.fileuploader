...
FileUploader.Model = Backbone.Model.extend({
  ...
  // add a progress event that can be fired during xhr
  _xhrProgressListener: function() {
    var xhr = $.ajaxSettings.xhr();
    var modelXhrProgressListener = _.bind(function(evt) {
      var progress = evt.lengthComputable ?  Math.ceil((evt.loaded / evt.total) * 100) : null;
      this.trigger('progress', this, progress);
    }, this);

    if (xhr instanceof window.XMLHttpRequest || xhr.upload) {
      xhr.addEventListener('progress', modelXhrProgressListener, false);
    }
  ...
  sync: function(method, model, options) {
    if (method === 'create' && this.browserFileObj) {
      _.extend(options, {
        data: this._buildForm(),
        processData: false,
        contentType: false,
        xhr: _.bind(this._xhrProgressListener, this)
      });
    }
